...
const ui32 clr_white = BGRA(0xff, 0xff, 0xff);
const ui32 clr_black = BGRA(0x00, 0x00, 0x00);
...
const bool g_iswhitekey[] = {
    1, 0, 1, 0, 1, /**/ 1, 0, 1, 0, 1, 0, 1};
const bool g_white_left[] = {
    0, 0, 1, 0, 1, /**/ 0, 0, 1, 0, 1, 0, 1};
const bool g_white_right[] = {
    ...
    while (processed_ticks < wanted_time) {
      ...
      for (k = 0; k < 128; k++) {
        ...
        if (nid_tmp != key_noteids[k]) {
          bar = mb->AddBar_pre(k, processed_ticks, end_old[k], nid_tmp > key_noteids[k]);
...
void MidiWhat::Main() {
  ...
  ui16 draw_h = FRAME_H * 9 / 10;
  for (b_u = 0; b_u <= viewport_height; b_u++)
    ys[b_u] = draw_h - b_u * draw_h / viewport_height;
  ui16 black_key_d = draw_h + FRAME_H * 1 / 15;
  si16 half_key_w = FRAME_W / 128 / 2;
    ...
    if (play_time_us == next_frame_us) {
      ...
      for (k = 0; k < 128; k++) {
        ...
        if (iswhite)
          ...
          color = clr_black;
        if (bar = mb->kbar[k].head.next)
          if (bar->bar_beg <= bottom_tick) {
            if (bar->noteid != -1)
              color = colors[bar->track % ncolors];
          }
        u = d + 1;
        ...
        if (iswhite) {
          ...
          if (g_white_left[chord_k])
            if ((l -= half_key_w) < 0)
              l = 0;
          if (g_white_right[chord_k])
            if ((r += half_key_w) > FRAME_W)
              r = FRAME_W;
          for (y = u; y < d; y++) {
            for (x = l; x < r; x++)
              frame[y * FRAME_W + x] = color;
          }
          if (chord_k == 0 || chord_k == 5) // two white key
            u = draw_h + 1;
          if (l > 0)
            for (y = u; y < d; y++)
              frame[y * FRAME_W + l - 1] = clr_black;
        }
      }
